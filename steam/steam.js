...
onGet('electron error',(err)=>{
	console.log(err);
	Game.Notify('<span class="warning">Electron error</span>','<span class="warning">'+err+'</span>',[32,17]);
});
...
Steam.modsPopup=function()
{
	/*
		...
		-open Steam workshop
		-open /mods folder
		-list of mods, which affect a section on the right depending on which is selected:
			-disable/enable
			-open location
			-view on Workshop (if workshop)
			-update mod (if workshop)
			-unsubscribe (if workshop)
			-priority up
			-priority down
			-details: file size, is local,...
	*/
	let selectedMod=0;
	...
	let checkModDependencies=(mod)=>{
		...
		if (mod.dependencies.length>0)
		{
			...
			{
				if (loadedMods.includes(mod.dependencies[ii])) mod.dependenciesStr.push(mod.dependencies[ii]);
				else {mod.dependenciesStr.push('<span style="color:#f00;display:inline-block;" class="pucker">'+mod.dependencies[ii]+'</span>');mod.disabled=true;okay=false;}
			}
	...
	let updateModOptions=()=>
	{
		...
		if (selectedMod)
		{
			...
			if (mod.workshop)
			{
				...
				//AddEvent(l('modUnsubscribeWorkshop'),'click',()=>{changeMods();send({id:'unsubscribe workshop',loc:mod.workshop});});
			}
		}
...
Steam.workshopPopup=function()
{
	...
	/*
		if no mod selected:
			-"[select your mod's folder]" (see https://jaketrent.com/post/select-directory-in-electron)
			-some info maybe? (view sample mods, etc)
		once selected:
			-name
			-author
			-version
			-file size when zipped
			-desc
			-image
			-tags
			-visibility
			-[publish to Workshop]
		show errors for improper mod details, upload fail, etc
	*/
	Game.Prompt('<h3>'+loc("Publish mods")+'</h3><div class="block" style="font-size:11px;">'+str+'</div>',[loc("Back")],0,'widePrompt');
}
//setTimeout(Steam.modsPopup,1000);
